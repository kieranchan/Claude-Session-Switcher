...
* **🎨 现代化 UI**: 采用卡片式设计，配备 SVG 图标和响应式布局，支持**深色模式**。
* **📥 智能自动获取**: 
    * 自动从当前标签页抓取 `sessionKey`。
    * **智能用户名提取**: 利用 DOM 技术直接从 Claude 侧边栏读取并填入您的用户名。
* **⏳ 限制追踪**: 
    * **高性能检测**: 使用 `TreeWalker` 和 `MutationObserver` 算法，以极低的资源消耗精准捕捉“消息额度已满”提示，拒绝页面卡顿。
    * **手动计时**: 可以手动为账号设置冷却倒计时。
* **🌍 网络监控**: 
    * 底部状态栏实时显示当前 **IP** 和 **地理位置**。
    * **一键体检**: 专用按钮跳转至外部服务，评估当前 IP 的风险指数（欺诈分数）。
* **✏️ 弹窗编辑**: 在简洁的模态框（遮罩层）中添加或修改账号，操作更专注。
* **✍️ 内联编辑**: 点击账号名旁的编辑按钮，直接在原处修改名称。
...
* **架构**: 
    * **Manifest V3**: 符合 Chrome 最新安全标准。
    * **原生高性能**: 不依赖庞大的框架，体积小巧，运行飞快。
* **算法优化**:
    * **TreeWalker**: DOM 遍历优化。
    * **Debounce**: 输入防抖。
    * **Hash Map**: 快速数据检索。
...
3.  **手动方式**: 将 `sk-ant...` 开头的 Key 粘贴到输入框。
4.  点击 **保存**。
